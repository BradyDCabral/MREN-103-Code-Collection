...
bool rightSeesLine() {
  return rightLineValue > LINE_THRESHOLD_R;
}
...
void followLineForward() {
  if (rightSeesLine() && leftSeesLine()) {
    ...
    beginStage((stage == MOVE_TO_PICKUP ? RECENTER_AFTER_PICKUP_TURN : RECENTER_AFTER_DUMP_TURN));
  }
  else if (rightSeesLine()) {
    setSingleLED(PIN_LED_YELLOW);
